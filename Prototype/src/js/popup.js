...
document.addEventListener('DOMContentLoaded', function () {
	...
	chrome.tabs.query({
		...
		chrome.tabs.sendMessage(tabs[0].id, JSON.stringify(data), initialiseAndShowInitialPage);
	});
	...
	function initialiseAndShowInitialPage(response) {
		if (response) {
			...
			auctionTimeData = collectAuctionData(allBidsDf, allAuctionsDf.groupBy('auction'));
	...
	// ***********************************************
	// * OLD
	// function collectAuctionData(bids, auctionId, slotElementId) {
	// 	let auctionBidData = bids.filter(row => row.get('auction') == auctionId);
	// 	let auctionBiddedData = auctionBidData.filter(row => row.get('type') == 'bid');
	// 	numAds = auctionBidData.distinct('adUnitPath').count();
	// 	numBidders = auctionBidData.distinct('bidder').count();
	// 	totalBids = auctionBidData.filter(row => row.get('type') == 'bid').count();
	// 	numMatchedBidders = auctionBiddedData.distinct('bidder').count();
	// 	numNoBidBidders = auctionBidData.filter(row => row.get('type') == 'noBid').distinct('bidder').count();

	// 	let auctionData = allAuctionsDf.filter(row => row.get('auction') == auctionId).getRow(0)
		
	// 	let auctionSlotData = allSlotsDf.join(auctionBiddedData.distinct('slotElementId'), ['slotElementId']).select('slotRenderedTs', 'slotLoadTs');
	// 	let minPostAuctionTs = auctionSlotData.stat.min('slotRenderedTs');
	// 	let maxPostAuctionTs = auctionSlotData.stat.max('slotLoadTs');
	// 	let preAuctionStartTs = auctionData.get('preAuctionStartTime');
	// 	let auctionStartTs = auctionData.get('startTime');
	// 	let auctionEndTs = auctionData.get('endTime');
	// 	let preAuctionTime = auctionStartTs - preAuctionStartTs;
	// 	let auctionTime = auctionEndTs - auctionStartTs;
	// 	let adAuctionTime = minPostAuctionTs - auctionEndTs;
	// 	let adRenderTime = maxPostAuctionTs - minPostAuctionTs;
	// 	auctionBidders.push([auctionId, slotElementId, numAds, totalBids, numBidders, numNoBidBidders, numMatchedBidders, preAuctionTime, auctionTime, adAuctionTime, adRenderTime]);
	// }
