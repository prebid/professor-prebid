...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.getElementById('auction').addEventListener('click', onclickAuction, false);
	document.getElementById('request').addEventListener('click', onclickRequest, false);
	document.getElementById('gpt').addEventListener('click', onclickGpt, false);
	...
	chrome.tabs.query({
		...
		chrome.tabs.sendMessage(tabs[0].id, JSON.stringify(data), processResponse);
	});
	...
	function sendMessage(evt, x) {
		...
		chrome.tabs.query({
			...
			chrome.tabs.sendMessage(tabs[0].id, JSON.stringify(data), function() {
				console.log('PREBID_TOOLS: sendMessage(' + evt + ' ' + x + ')');
			});
		});
	}
