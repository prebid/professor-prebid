...
class AdMaskComponent extends React.Component<{ input: IMaskInputData }> {
    render() {
        const { prebid, creativeRenderTime, elementId, winningCPM } = this.props.input;
                ...
                {auctionEndEvents?.map(event =>
                    <ul key={event.args.auctionId}>
                        ...
                        <li key="Bidders">
                            ...
                            <ul>
                                {Array.from(new Set(prebid.bids?.map(bidder => bidder.bidder))).map(bidder => <li key={bidder as string}>{bidder}</li>)}
                            </ul>
                        </li>
                    </ul>
                )}
