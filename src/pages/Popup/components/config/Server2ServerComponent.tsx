...
const Server2ServerComponent = ({ prebid }: Server2ServerComponentProps): JSX.Element => {
  ...
  return (
    <Box sx={{ width: '100%', padding: '5px 5px 5px 25px' }}>
      ...
      {prebid.config.s2sConfig?.adapter && (
        <Typography>
          <strong>Adapter:</strong> {prebid.config.s2sConfig.adapter}
        </Typography>
      )}
      {prebid.config.s2sConfig?.adapterOptions && (
        <Typography>
          <strong>Adapter Options:</strong> {JSON.stringify(prebid.config.s2sConfig.adapterOptions)}
        </Typography>
      )}
      {prebid.config.s2sConfig?.maxBids && (
        <Typography>
          <strong>max. Bids:</strong> {prebid.config.s2sConfig.maxBids}
        </Typography>
      )}
      {prebid.config.s2sConfig?.syncUrlModifier && (
        <Typography>
          <strong>Sync. Url Modifier:</strong> {JSON.stringify(prebid.config.s2sConfig.syncUrlModifier)}
        </Typography>
      )}
      {prebid.config.s2sConfig?.timeout && (
        <Typography>
          {' '}
