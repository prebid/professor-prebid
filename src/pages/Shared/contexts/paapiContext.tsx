...
export const PaapiContextProvider = ({ children }: PaapiProviderProps) => {
  ...
  const [auctionState, setAuctionState] = useState<{ [key: string]: IAuctionInfo }>({
    global: { auctionId: 'global', auctionType: 'Top', auctionTime: '', eventTable: [], startTime: Date.now() / 1000 },
  });
  ...
  useEffect(() => {
    ...
    const handleMessage = (message: IMessage) => {
      ...
      const { uniqueAuctionId, type, parentAuctionId = 'global', auctionConfig, requestId } = params;
      const auctionId = uniqueAuctionId || 'global';
      const eventRow = eventRowFromMessage(message, auctionState[auctionId]);

      if (method === 'Page.frameNavigated') {
        // console.log('message', message);
        // Clear out all auction state on navigation.
        for (const key of Object.keys(auctionState)) {
          if (key !== 'global') {
      ...
      if (method === 'Storage.interestGroupAuctionEventOccurred') {
        if (type === 'started') {
          const auctionInfo: IAuctionInfo = {
            auctionId,
            auctionType: parentAuctionId === 'global' ? 'Top' : 'Component',
            auctionTime: new Date(messageTime(message) * 1000).toISOString(),
            parentAuctionId,
  ...
  const toggleAuction = (auctionId: string) => {
    ...
    const auction = newAuctionState[auctionId];
    auction.expanded = !auction.expanded;
    setAuctionState(newAuctionState);
  };

  const contextValue: IPaapiInfo = { auctionState, setAuctionState, toggleAuction };
...
interface IPaapiInfo {
  ...
  setAuctionState?: React.Dispatch<React.SetStateAction<{ [key: string]: IAuctionInfo }>>;
  toggleAuction: (auctionId: string) => void;
}
